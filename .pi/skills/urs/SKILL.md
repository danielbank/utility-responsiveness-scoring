...
- **urs_fetch_edgar** — When the user wants to pull 10-K/10-Q filings from SEC EDGAR for an investor-owned utility. Fetches from SEC's free API, optionally ingests for scoring. Only works for utilities with a CIK mapping (see .pi/extensions/urs/data/edgar-cik-map.ts). APS (803), Duke (6452), Dominion (13998) are mapped. You can also pass a `cik` param directly to bypass the mapping.
